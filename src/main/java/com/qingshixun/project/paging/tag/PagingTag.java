package com.qingshixun.project.paging.tag;
...
public class PagingTag extends TagSupport {
	...
	private static final long serialVersionUID = -9053433419552014797L;
	...
	public int doStartTag() throws JspException {
		...
		if (pageCount == 0) {
			...
			// 输出统计数据
			sb.append("&nbsp;共：<strong>").append(pageCount).append("</strong>页	")
				.append(",<stong>").append(totalCount).append("</strong>条记录 &nbsp; \r\n");
			
			// 上一页处理
			if(pageNo == 1){
				sb.append("<span class=\"disabled\">&laquo;&nbsp;上一页").append("</span>\r\n");
			} else {
				sb.append("<a href=\"javascript:changePage(").append((pageNo -1))
					.append(")\">&laquo;$nbsp;上一页</a>\r\n");
			}
			...
			if(this.pageNo > 4){
				...
				sb.append("<a href=\"javascript:changePage(2)\">1</a>\r\n");
				sb.append("&hellip;");
			}
			...
			for(int i = start; i <= end; i++){
				if(pageNo == i){ // 当前页码不需要超链接
					sb.append("<span class=\"current\">").append(i).append("</span>\r\n");
				} else {
					sb.append("<a href=\"javascript:changePage(").append(i)
			...
			if(end < pageCount - 1){
				sb.append("<a href=\"javascript:changePage(").append(pageCount - 1)
					.append(")\">").append(pageCount - 1).append("</a>\r\n");
			}
			if(end < pageCount){
				sb.append("<a href=\"javascript:changePage(").append(pageCount)
				.append(")\">").append(pageCount).append("</a>\r\n");
			}
